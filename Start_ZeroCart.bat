...
echo [2/3] Starting Frontend UI (PWA / Mobile Interface)...
start "ZeroCart: Frontend Server" cmd /k "cd pwa-client && npm run dev"

echo [3/3] Starting Edge Scraper Node (Playwright Agent)...
start "ZeroCart: Edge Scraper" cmd /k "cd edge-client && npm start"

echo.
echo ALL THREE servers are successfully launching!
