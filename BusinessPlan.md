...
Split-Brain Accuracy: Utilizing Google OR-Tools for hard math ensures budgets are never accidentally exceeded due to LLM hallucinations, while Vertex AI (Gemini 1.5 Pro) handles the creative culinary and tool-calling aspects.
...
7.1 Layer 1: The Cloud Brain (Centralized Intelligence on GCP) The intelligence is split into two distinct engines to prevent LLM hallucinations.
The Deterministic Engine (Math & Budget): Running on Google Cloud Run, a containerized Python environment utilizes Google OR-Tools (advanced linear constraint programming). It ingests the local store's pricing data and the user's budget, solving complex equations (e.g., maximizing protein while minimizing cost across 40 ingredients) in under 0.2 seconds. It does not guess; it calculates.
The Probabilistic Engine (The Chef): Powered by Vertex AI (Gemini 1.5 Pro). Once OR-Tools establishes the exact raw ingredient list, Gemini processes this data through its large context window to generate strict JSON recipes. It references the user's 6-month eating history to prevent meal fatigue.
